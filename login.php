...
function createUserSession($conn, $userId) {
    ...
    $stmt = $conn->prepare("INSERT INTO user_sessions (user_id, session_id, ip_address, user_agent, created_at, last_active_at, is_active) VALUES (?, ?, ?, ?, NOW(), NOW(), 1) ON DUPLICATE KEY UPDATE last_active_at = NOW(), is_active = 1");
    if ($stmt) {
...
try {
    ...
    // allow login by email OR username using the same input field
    $stmt = $conn->prepare("SELECT id, username, full_name, email, password, is_verified, role, is_suspended, is_deleted FROM users WHERE email = ? OR username = ? LIMIT 1");
    ...
    if ($row = $result->fetch_assoc()) {
        if (password_verify($password, $row["password"])) {
            ...
            $adminLoginOnly = isset($postData['admin_login']) && intval($postData['admin_login']) === 1;
